...
void AOnlineRacingMatchGameMode::BeginPlay()
{
	...
	{
		AOnlineRacingPlayerState* const RacePlayerState = Cast<AOnlineRacingPlayerState>(PlayerState);
...
void AOnlineRacingMatchGameMode::PostLogin(APlayerController* NewPlayer)
{
	...
	AOnlineRacingPlayerState* const RacePlayerState = NewPlayer->GetPlayerState<AOnlineRacingPlayerState>();
	if (IsValid(RacePlayerState))
	{
		InitializePlayerState(*RacePlayerState);
	}
	...
	const AOnlineRacingPawn& VehiclePawn)
{
	...
	AOnlineRacingPlayerState* const RacePlayerState = VehiclePawn.GetPlayerState<AOnlineRacingPlayerState>();
...
bool AOnlineRacingMatchGameMode::HandleRespawnRequest(AController& Controller)
{
	...
	const AOnlineRacingPlayerState* const RacePlayerState = Controller.GetPlayerState<AOnlineRacingPlayerState>();
...
void AOnlineRacingMatchGameMode::InitializePlayerState(AOnlineRacingPlayerState& RacePlayerState) const
{
	RacePlayerState.InitializeRaceProgress(1);
}

void AOnlineRacingMatchGameMode::RecordPlayerFinish(AOnlineRacingPlayerState& RacePlayerState)
{
	AOnlineRacingMatchGameState* const RaceGameState = GetGameState<AOnlineRacingMatchGameState>();
	if (!IsValid(RaceGameState))
	{
...
bool AOnlineRacingMatchGameMode::AreAllPlayersFinished() const
{
	...
	{
		const AOnlineRacingPlayerState* const RacePlayerState = Cast<AOnlineRacingPlayerState>(PlayerState);
		if (!IsValid(RacePlayerState))
		{
			continue;
		}
